...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        self._NoOfPieces = NoOfPieces #LINE ADDED
        self._Board = []
        self._Players = []
    ...
    def __CheckReincarnation(self, SquareReference):
        ...
        if self._CurrentPlayer.SameAs(self._Players[0]):
            if Row == self._NoOfRows and self.__CountNormalPieces() < self._NoOfPieces:
                print("Congratulations, you have earned a reincarnation!")
                ReincarnationCol = int(input("Which column would you like your piece to be reincarnated on? "))
                while self._Board[self.__GetIndexOfSquare(10+ReincarnationCol)].GetPieceInSquare() != None:
                    print("The square must be empty.")
                    ReincarnationCol = int(input("Which column would you like your piece to be reincarnated on? "))
                self._Board[self.__GetIndexOfSquare(10+ReincarnationCol)].SetPiece(Piece("piece", self._Players[0], 1, "!"))
        else:
            if Row == 1 and self.__CountNormalPieces() < self._NoOfPieces:
                print("Congratulations, you have earned a reincarnation!")
                ReincarnationCol = int(input("Which column would you like your piece to be reincarnated on? "))
                while self._Board[self.__GetIndexOfSquare(6*10+ReincarnationCol)].GetPieceInSquare() != None:
                    print("The square must be empty.")
                    ReincarnationCol = int(input("Which column would you like your piece to be reincarnated on? "))
                self._Board[self.__GetIndexOfSquare(6*10+ReincarnationCol)].SetPiece(Piece("piece", self._Players[1], 1, '"'))
    #END ADDITION
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                self.__CheckReincarnation(FinishSquareReference) #LINE ADDED
