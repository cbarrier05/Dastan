    ...
    def __WeatherEventOccurs(self):
        ...
        if random.randint(1,2) == 1:
            ...
            while not squareFound:
                randomSquare = random.randint(1,self._NoOfRows)*10 + random.randint(1,self._NoOfColumns)
                if self._Board[self.__GetIndexOfSquare(randomSquare)].GetPieceInSquare() == None and not self._Board[self.__GetIndexOfSquare(randomSquare)].ContainsKotla():
                    squareFound = True
            weatherEvent=WeatherEvent(randomSquare)
        return weatherEvent
    #END ADDITION

    def PlayGame(self):
        GameOver = False
        weatherEvent = None #LINE ADDED
        while not GameOver:
            self.__DisplayState()
            SquareIsValid = False
            Choice = 0
            ...
            if weatherEvent==None:
                weatherEvent = self.__WeatherEventOccurs()
            else:
                if weatherEvent.CountDownComplete():
                    ColToDestroy = weatherEvent.GetWeatherLocation()%10
                    for Row in range (1,self._NoOfRows+1):
                        if self._Board[(self.__GetIndexOfSquare(Row*10+ColToDestroy))].GetPieceInSquare() != None:
                            self._Board[(self.__GetIndexOfSquare(Row*10+ColToDestroy))].RemovePiece()
                        if self._Board[(self.__GetIndexOfSquare(Row*10+ColToDestroy))].ContainsKotla():
                            self._Board[(self.__GetIndexOfSquare(Row*10+ColToDestroy))] = Square()
            #END ADDITION
...
#CODE ADDED
class WeatherEvent():
    def __init__(self, SquareReference):
        self.__SquareReference = SquareReference
        self.__CountDownTimer = 3
        print("A weather event has occured at location",SquareReference)
        print("After two complete turns, all pieces on the same column will be destroyed.")

    def SetWeatherLocation(self, SquareReference):
        self.__SquareReference = SquareReference

    def GetWeatherLocation(self):
        return self.__SquareReference

    def CountDownComplete(self):
        if self.__CountDownTimer == 0:
            print("The weather event destroys all the pieces in column",self.__SquareReference%10)
            return True
        else:
            self.__CountDownTimer -= 1
            print("The weather event at location", self.__SquareReference,"will occur", "after one more turn" if self.__CountDownTimer>1 else "next turn")
            return False
#END ADDITION
#         
class Kotla(Square):
