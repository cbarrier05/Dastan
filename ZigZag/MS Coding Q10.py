...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            Choice = 0
            while Choice < 1 or (Choice > 3 and Choice != 7):
                Choice = int(input("Choose move option to use from queue (1 to 3), 7 to create a Kotla or 9 to take the offer: "))
                if Choice == 9:
            ...
            if Choice==7:
                while not SquareIsValid:
                    StartSquareReference = self.__GetSquareReference("containing the piece to sacrifice for a new Kotla")
                    SquareIsValid = self.__CheckSquareIsValid(StartSquareReference, True)
                if self._CurrentPlayer.SameAs(self._Players[0]):
                    self._Board[self.__GetIndexOfSquare(StartSquareReference)] = Kotla(self._CurrentPlayer, "K")
                else:
                    self._Board[self.__GetIndexOfSquare(StartSquareReference)] = Kotla(self._CurrentPlayer, "k")
            else:
                while not SquareIsValid:
