...
class Dastan:
    ...
    #CODE ADDED
    def __CalculateSahmMove(self, SquareReference):
        Row = SquareReference // 10
        Col = SquareReference % 10
        Score = 0
        Direction = self._CurrentPlayer.GetDirection()
        if Direction == 1:
            EndRow = 6
        else:
            EndRow = 1
        while Row != EndRow:
            Row += Direction
            BoardSquare = self._Board[self.__GetIndexOfSquare(10*Row+Col)]
            if BoardSquare.GetPieceInSquare() is not None and not BoardSquare.ContainsKotla():
        ...
        while not GameOver:
            ...
            if self._CurrentPlayer.ChoiceIsSahm(Choice):
                if self._CurrentPlayer.GetSahmUsed():
                    ...
                    PointsForPieceCapture = self.__CalculateSahmMove(StartSquareReference)
    ...
    def __CreateMoveOptionOffer(self):
        self._MoveOptionOffer.append("sahm") #LINE ADDED
...
class Player:
    ...
    def CheckPlayerMove(self, Pos, StartSquareReference, FinishSquareReference):
        Choice = self.__Queue.GetMoveOptionInPosition(Pos - 1)
        return Choice.CheckIfThereIsAMoveToSquare(StartSquareReference, FinishSquareReference)
