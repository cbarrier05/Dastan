...
class Dastan:
    ...
    #CODE ADDED
    def __CheckBarrierIsValid(self, BarrierCentre):
        BarrierIsValid = True
        BarrierRow = BarrierCentre // 10
        BarrierCol = BarrierCentre % 10
        for col in range(BarrierCol-1, BarrierCol+2):
            if not self.__CheckSquareInBounds(BarrierRow*10+col):
                BarrierIsValid = False
            else:
                if self._Board[self.__GetIndexOfSquare(BarrierRow*10+col)].ContainsKotla() or self._Board[self.__GetIndexOfSquare(BarrierRow*10+col)].ContainsBarrier() or self._Board[self.__GetIndexOfSquare(BarrierRow*10+col)].GetPieceInSquare() != None:
                    BarrierIsValid = False
        return BarrierIsValid

    def __PlaceBarrier(self, P, S):
        BarrierValid = False
        while not BarrierValid:
            BarrierSquare = int(input("Which square would you like to be the centre of your horizontal, 3-square-wide barrier? "))
            if self.__CheckBarrierIsValid(BarrierSquare):
                BarrierValid = True
            else:
                print("You must choose the centre of 3 empty squares on the board horizonally.")
        BarrierRow = BarrierSquare // 10
        BarrierCol = BarrierSquare % 10
        for col in range(BarrierCol-1, BarrierCol+2):
            self._Board[self.__GetIndexOfSquare(BarrierRow*10+col)] = Barrier(P,S)
    #END ADDITION
    ...
    #CODE ADDED
    def __CheckManhattanDistance(self, Choice, StartSquare, EndSquare):
        if self._CurrentPlayer.CheckPlayerMove(Choice,StartSquare,EndSquare):
            HorizontalDirection = -1 if EndSquare%10<StartSquare%10 else 1
            VerticalDirection = -1 if EndSquare//10<StartSquare//10 else 1
            route1Valid = True
            for Row in range(StartSquare//10 + VerticalDirection, EndSquare//10 + VerticalDirection, VerticalDirection):
                if self._Board[self.__GetIndexOfSquare(Row*10+StartSquare%10)].ContainsBarrier():
                    route1Valid = False
            for Col in range(StartSquare%10, EndSquare%10, HorizontalDirection):
                if self._Board[self.__GetIndexOfSquare(EndSquare//10 * 10 + Col)].ContainsBarrier():
                    route1Valid = False
            
            route2Valid = True
            for Col in range(StartSquare%10 + HorizontalDirection, EndSquare%10 + HorizontalDirection, HorizontalDirection):
                if self._Board[self.__GetIndexOfSquare(StartSquare//10+Col)].ContainsBarrier():
                    route2Valid = False
            for Row in range(StartSquare//10, EndSquare//10, VerticalDirection):
                if self._Board[self.__GetIndexOfSquare(Row * 10 + EndSquare%10)].ContainsBarrier():
                    route2Valid = False
            return route1Valid or route2Valid
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            #CODE CHANGE
            MoveLegal = self.__CheckManhattanDistance(Choice, StartSquareReference, FinishSquareReference)
            #END CHANGE
            if MoveLegal:
    ...
    def __CreatePieces(self, NoOfPieces):
        ...
        #CODE CHANGE
        print("Player One, it's time to place your barrier")
        self.__PlaceBarrier(self._Players[0],"B")
        for Count in range(1, NoOfPieces + 1):
        ...
        print("Player Two, it's time to place your barrier")
        self.__PlaceBarrier(self._Players[1],"b")
        #END CHANGE
