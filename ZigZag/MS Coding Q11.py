...
class Dastan:
    ...
    def __ModifyQueueOptions(self):
        print("You have the following options to modify your queue:")
        print("a) Reverse your queue")
        print("b) Swap queues with your opponent")
        print("c) Swap the first and last move options in your queue")
        print("d) Move an option of your choice to position 1 in the queue")
        print("e) Don't modify the queue, let me play my move!")
        option = input("Enter your choice (a-e): ").lower()
        while option not in ["a","b","c","d","e"]:
            print("You must choose a letter from a to e: ", end="")
            option = input().lower()
        if option == "e":
            return
        elif option == "a":
            ...
            p1Queue = self._Players[0].ReplaceQueue(None)
            p2Queue = self._Players[1].ReplaceQueue(p1Queue)
            self._Players[0].ReplaceQueue(p2Queue)
        elif option == "c":
            self._CurrentPlayer.SwapFirstAndLast()
        elif option == "d":
            self._CurrentPlayer.MoveItemToFront(int(input("Enter position of item to move to the front(2-5): ")))
...
class Player:
    ...
    def ReplaceQueue(self, NewQueue):
        old = self.__Queue
    ...
    def MoveItemToFront(self, ItemNumber):
        self.__Queue.MoveItemToFront(ItemNumber-1)
    #END ADDITION
