...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            undo = "n"
            if MoveLegal:
                PreviousScore = self._CurrentPlayer.GetScore()
                ...
                self.__DisplayState()
                undo = input("Would you like to undo your move (y/n)? ")
                if undo.strip().lower() == "y":
                    self.__UpdateBoard(FinishSquareReference,StartSquareReference)
                    self._CurrentPlayer.ResetQueueBackAfterUndo(Choice)
                    self._CurrentPlayer.ChangeScore(PreviousScore-self._CurrentPlayer.GetScore()-5)
                else:
                    undo = "n"
            if undo == "n":
...
class MoveOptionQueue:
    ...
    def ResetQueueBack(self, Position):
        self.__Queue.insert(Position,self.__Queue.pop(len(self.__Queue)-1))
    #END ADDITION
...
class Player:
    ...
    #CODE ADDED
    def ResetQueueBackAfterUndo(self, Choice):
        self.__Queue.ResetQueueBack(Choice - 1)
    #END ADDITION
