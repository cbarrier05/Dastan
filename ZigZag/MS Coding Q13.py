...
class Dastan:
    ...
    def PlayGame(self):
        GameOver = False
        taziz = self._Board[self.__GetIndexOfSquare((self._NoOfRows // 2 + 1)*10 + (self._NoOfColumns // 2 + self._NoOfColumns % 2))]
        while not GameOver:
            self.__DisplayState()
            SquareIsValid = False
            Choice = 0
            ...
            if MoveLegal:
                ...
                taziz.CheckCamp()
                if not taziz.GetCampedTwoTurns():
...
class Taziz(Square):
    ...
    def CheckCamp(self):
        if self._PieceInSquare != None:
          if (self._PieceInSquare.GetBelongsTo().GetDirection() == 1 and self._Symbol == "A") or (self._PieceInSquare.GetBelongsTo().GetDirection() == -1 and self._Symbol == "a"):
              self._CampedTurns += 1

    def GetCampedTwoTurns(self):
        return self._CampedTurns >= 2
